...
int main(int argc, char *argv[])
{
    ...
    char *reste;
    int n = atoi(argv[1]);
    double d = strtod(argv[2],&reste);
    int max_wgt = atoi(argv[3]);
    char *nom_sortie = argv[4];
    
    int e = (d*n*(n-1))/2;
    
    if (e < n-1)
    {
        puts("Densité trop faible pour générer un graphe connexe !");
        exit(0);
    }
    random_connected_graph(n,e,max_wgt,1,nom_sortie);
}
